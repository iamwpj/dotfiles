...
#
# Shamefully copied from https://github.com/Pipshag/dotfiles_gruvbox/blob/master/.config/waybar/custom_modules/custom-kdeconnect.sh
...
send_clipboard() {
    for device in $(qdbus --literal org.kde.kdeconnect /modules/kdeconnect org.kde.kdeconnect.daemon.devices); do
        ...
        if [ "$isreach" = "true" ] && [ "$istrust" = "true" ]
        then
            $(qdbus org.kde.kdeconnect "/modules/kdeconnect/devices/$deviceid/clipboard" org.kde.kdeconnect.device.clipboard)
        fi
    done
...
option="${1}"
    case "${option}" in
        ...
        test)
            echo '#655b53' 
            ;;
        *) echo -e "Use with ./custom-kdeconnect.sh [OPTION]\n OPTIONS:\n\tring\n\tbattery\n\tclipboard"
            ;;
    esac
